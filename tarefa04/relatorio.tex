...
O arquivo é compilado com GCC, \texttt{-fopenmp} e \texttt{-lm}. O programa recebe o número de elementos como argumento e usa $N=50.000.000$ como valor padrão. Os testes deste relatório usam $N=5.000.000$. Para cada quantidade de threads, o programa é executado três vezes e a mediana de cada teste é registrada. A medição usa \texttt{CLOCK\_MONOTONIC}, que não é afetado por alterações no relógio do sistema.
...
\caption{Tempo de execução em função do número de threads. Os tempos são as medianas de três execuções.}
\label{fig:tempos}
\end{figure}
...
Os resultados dependem do processador, da memória, da afinidade das threads e da carga do sistema. Portanto, a posição exata do ponto de saturação não deve ser tratada como uma propriedade universal. Para estudar especificamente SMT, é necessário conhecer a quantidade de núcleos físicos e controlar a afinidade com \texttt{OMP\_PROC\_BIND} e \texttt{OMP\_PLACES}.

\section{Conclusão}
Os dois testes paralelos usam OpenMP e permitem comparar dois tipos de gargalo. O paralelismo reduz o tempo quando há trabalho independente e recursos disponíveis. No caso memory-bound, o limite aparece quando a memória não consegue fornecer dados mais rapidamente. No caso compute-bound, o limite aparece quando as unidades de execução da CPU ficam ocupadas. O uso de threads deve ser escolhido com base no recurso que limita a aplicação.
