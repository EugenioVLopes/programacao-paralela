...
\begin{center}
    {\LARGE\bfseries Escopo de variáveis e regiões críticas com OpenMP}\\[0.4em]
...
O quadrado de lado 2 tem área 4 e o círculo unitário inscrito tem área $\pi$. Se os pontos forem uniformes no quadrado, a fração de pontos dentro do círculo converge para a razão entre as áreas:
\[
...
\]
Cada ponto $(x,y)$ com $x,y \in [-1,1]$ pertence ao círculo quando $x^2 + y^2 \leq 1$. Como o método usa amostras aleatórias, o resultado é uma aproximação. Aumentar $N$ reduz a variação, mas não elimina o caráter estocástico da estimativa.

\subsection{Condição de corrida}
A operação \texttt{dentro\_circulo++} não é atômica no nível da máquina. O processador executa três passos separados: ler o valor da memória para um registrador (LOAD), incrementar o registrador (ADD) e escrever o resultado de volta (STORE). Se duas threads executam o LOAD antes que qualquer uma complete o STORE, ambas leem o mesmo valor, ambas incrementam para o mesmo resultado e uma das escritas sobrescreve a outra. Perde-se um incremento. O programa termina normalmente, mas o resultado final é menor que o correto.

A gravidade do problema depende do número de threads e da frequência dos acessos concorrentes. Não há como prever exatamente quantos incrementos serão perdidos em cada execução, pois o escalonamento das threads e a ordem das operações mudam a cada execução.

\subsection{Escopo de variáveis em OpenMP}
Em uma região paralela, o OpenMP define regras de visibilidade para cada variável:

\begin{itemize}
    \item \texttt{shared(var)}: todas as threads compartilham o mesmo endereço de memória. Alterações são visíveis a todas as threads, mas sem sincronização podem causar condições de corrida.
    \item \texttt{private(var)}: cada thread recebe uma cópia local da variável, sem valor inicial definido (conteúdo indeterminado). A variável externa não é modificada pela região paralela.
    \item \texttt{firstprivate(var)}: como \texttt{private}, mas a cópia local é inicializada com o valor que a variável tinha antes da região paralela.
    \item \texttt{lastprivate(var)}: aplicável a laços. Ao final do laço, a variável externa recebe o valor produzido na última iteração lógica, e não o valor da thread que terminou por último.
    \item \texttt{default(none)}: proíbe escopos implícitos. O compilador exige que toda variável referenciada dentro da região paralela seja explicitamente declarada com alguma cláusula de escopo. Variáveis esquecidas geram erro de compilação.
\end{itemize}
...
Para validar isoladamente as cláusulas de compartilhamento e escopo de dados em OpenMP, desenvolvemos quatro programas complementares:
\begin{itemize}
    \item \textbf{e4\_private.c} -- demonstra a criação de cópias locais sem inicialização;
    \item \textbf{e4\_firstprivate.c} -- demonstra a inicialização da cópia local com o valor prévio;
    \item \textbf{e4\_lastprivate.c} -- demonstra a propagação do valor da última iteração lógica;
    \item \textbf{e4\_default\_none.c} -- demonstra a obrigatoriedade de classificação explícita.
\end{itemize}
...
Compilamos os códigos-fonte com GCC 13.3, padrão C11, nível de otimização \texttt{-O2}, avisos ativados (\texttt{-Wall -Wextra}) e suporte a OpenMP (\texttt{-fopenmp}). Para as medições de desempenho, a afinidade das threads é controlada por variáveis de ambiente (\texttt{OMP\_PROC\_BIND=close} e \texttt{OMP\_PLACES=cores}), variando a contagem de threads de 1 até o limite da máquina com dez repetições por configuração.
...
gcc -std=c11 -O2 -Wall -Wextra -fopenmp e1.c -o e1 -lm
gcc -std=c11 -O2 -Wall -Wextra -fopenmp e2.c -o e2 -lm
gcc -std=c11 -O2 -Wall -Wextra -fopenmp e3.c -o e3 -lm

OMP_NUM_THREADS=28 ./e1 5000000
OMP_NUM_THREADS=28 ./e2 5000000
...
Realizamos os testes com $N=5.000.000$, dez repetições por configuração, em um Intel Core i7-14700HX exposto ao WSL2 com 14 núcleos físicos e 28 threads lógicas. A afinidade foi configurada com \texttt{OMP\_PROC\_BIND=close} e \texttt{OMP\_PLACES=cores}, e \texttt{OMP\_DYNAMIC} ficou desativado. A máquina é um laptop sob um hipervisor -- o sistema operacional interfere nas medições e o desvio padrão é alto em várias configurações. As tabelas usam a mediana dos tempos; as barras de erro dos gráficos mostram o desvio padrão das dez repetições.

\subsection{Versão sem sincronização}
Com uma thread, e1.c retornou $\hat{\pi}=3{,}1415096$, igual ao resultado de e2.c. Os dois programas usam a mesma sequência de pontos, portanto a diferença a partir de duas threads só pode vir do acesso concorrente ao contador. A Tabela \ref{tab:race} resume as dez execuções de e1.c. Com 2 threads, a média já cai para 2,06 (erro de 34\%). Com 4 threads, cai para 0,91 (erro de 71\%). Com 8 threads ou mais, a média oscila entre 0,38 e 0,62, sempre muito abaixo do valor correto.

\begin{table}[H]
\centering
\caption{Estimativa de $\pi$ em e1.c com race condition (10 execuções por configuração).}
\label{tab:race}
\begin{tabular}{crrr}
\hline
...
A Tabela \ref{tab:tempos} compara os tempos de e2.c e e3.c. Em e2.c, o tempo mediano subiu de 0,096032 s com uma thread para 1,221722 s com 28. O trabalho geométrico é paralelo, mas aproximadamente $\pi N/4$, ou 3,93 milhões de pontos, entram no círculo e disputam a seção \texttt{critical}. Com mais threads, a disputa cresce e o tempo flutua entre 0,9 e 1,5 s desde a oitava thread, com desvios padrão altos. O speedup fica abaixo de 1 em todas as configurações com mais de uma thread: a paralelização \emph{piora} o desempenho.
...
Executamos quatro programas complementares para avaliar o comportamento isolado de cada cláusula de escopo:
...
\section{Conclusão}
A versão e1.c mostra que um contador compartilhado não pode receber incrementos concorrentes sem sincronização: o programa compila, executa e imprime um valor, mas a estimativa de $\pi$ com 8 threads cai para 0,46 contra o valor correto de 3,14. e2.c preserva a contagem, mas serializa milhões de atualizações e perde desempenho conforme o número de threads aumenta. e3.c acumula localmente e sincroniza apenas no final. Com 14 threads, a versão levou 0,003498 s, contra 0,746605 s de e2.c na mesma configuração -- o speedup atinge 16$\times$, contra 0,13$\times$ de e2.c. O teste também mostra o limite prático do hardware: depois dos 14 núcleos físicos, as threads lógicas não melhoraram o tempo de e3.c e ainda aumentaram a variância das medições.

As cláusulas \texttt{private}, \texttt{firstprivate}, \texttt{lastprivate} e \texttt{default(none)} controlam o escopo e a visibilidade das variáveis. A cláusula \texttt{default(none)} se torna útil em programas complexos porque obriga a declaração explícita de todas as variáveis, prevenindo erros sutis de compartilhamento acidental.
