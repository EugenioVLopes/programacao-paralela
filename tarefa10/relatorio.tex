...
\begin{quote}
``Implemente novamente o estimador da tarefa 8 usando um contador
compartilhado e o \texttt{rand\_r} protegendo a soma de hits com
\texttt{\#pragma omp critical} e com \texttt{\#pragma omp atomic}.
Compare essas duas implementações com suas versões que usam contadores
privados. Agora, compare essas com uma 5ª versão que utiliza apenas a
cláusula \texttt{reduction} ao invés das diretivas de sincronização.
Reflita sobre a aplicabilidade de desses mecanismos em termos de desempenho
e produtividade e proponha um roteiro para quando utilizar qual mecanismo
de sincronização, incluindo \texttt{critical} nomeadas e locks explícitos.''
\end{quote}

Portanto, são comparadas duas versões com contador compartilhado, duas
com contador privado e uma quinta versão com \texttt{reduction}.
...
Os contadores privados com \texttt{critical} final ou \texttt{atomic}
final representam formas manuais do mesmo padrão. Em ambos os casos,
cada thread sincroniza uma única vez, totalizando $O(p)$ atualizações,
em vez das $O(N)$ atualizações das versões compartilhadas.
...
Foram utilizadas cinco versões em C11, todas com $N=2.000.000$
definido diretamente no código, cláusula \texttt{default(none)} e
...
\item v4: contador privado com \texttt{atomic} uma única vez;
...
\newpage
\begin{lstlisting}[language=bash,caption={Compilação das cinco versões}]
...
0,0323~s com 1 thread para 0,6883~s com 28 threads.

A versão v2 passou de 0,0187~s para 0,0738~s no mesmo intervalo. A
operação atômica apresenta custo inferior ao de \texttt{critical}, mas
ainda ocorre sincronização a cada hit.

As versões v3, v4 e v5 apresentaram os menores tempos e escalaram bem
enquanto havia núcleos disponíveis. Em v3 e v4, a privatização reduz a
frequência da sincronização de uma vez por hit para uma vez por thread;
a diferença isolada entre elas é o mecanismo da combinação final:
\texttt{critical} em v3 e \texttt{atomic} em v4.
...
tempo semelhante ao das versões privadas v3 e v4 com uma implementação
mais compacta, além de facilitar a leitura do código.

\texttt{atomic} apresentou desempenho superior ao de \texttt{critical}
quando utilizado por hit, aproximadamente 9 vezes melhor com
...
\lstinputlisting[language=C,caption={v4: contador privado com atomic final. Custo O(p): acumula sem sincronização e combina atomicamente uma vez por thread.},label={lst:v4}]{v4_private_atomic_randr.c}
